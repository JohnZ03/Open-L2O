...
# ---------------------------------------
def confocal_microscopy_3d(batch_size=128, num_points=5, ROI=[28, 28, 28], stddev=0.01, dtype=tf.float32,
                           inference=False):
    if inference:
        def build():
            """Builds loss graph."""
        ...
        '''
                ...
                I = I0 * ((-tf.math.erf((-0.5 - x0 + xk) / (tf.math.sqrt(2.0) * sigmaxy)) + tf.math.erf(
                    (0.5 - x0 + xk) / (tf.math.sqrt(2.0) * sigmaxy))) \
                          * (-tf.math.erf((-0.5 - y0 + yk) / (tf.math.sqrt(2.0) * sigmaxy)) + tf.math.erf(
                            (0.5 - y0 + yk) / (tf.math.sqrt(2.0) * sigmaxy))) \
                          * (-tf.math.erf((-0.5 - z0 + zk) / (tf.math.sqrt(2.0) * sigmaz)) + tf.math.erf(
                            (0.5 - z0 + zk) / (tf.math.sqrt(2.0) * sigmaz)))) / 8.0
                return I
            ...
            obj = tf.reduce_mean(
                tf.math.reduce_sum((y_pred + bg_var - tf.math.l2_normalize(img_placeholder, axis=1)) ** 2, axis=1))
            return obj
    else:
        def build():
            """Builds loss graph."""
        ...
        '''
            ...
            bg_sim = tf.get_variable(
                ...
                trainable=False)
            obj = tf.reduce_mean(
                tf.math.reduce_sum((y_pred + bg_var - tf.math.l2_normalize(y_sim + bg_sim, axis=1)) ** 2, axis=1))
            return obj
    return build
...
def pzf_opt(N=5, K=2, dtype=tf.float32):
    def build():
        ...
        # Non-trainable variables.
        # Generate ci, di and pend to list C, D
        C = []
        D = []

        for i in range(N):
            ci = tf.get_variable("c{}".format(i),
                                 shape=[N - K, 1],
                                 dtype=dtype,
                                 initializer=tf.random_normal_initializer(),
                                 trainable=False)

            di = tf.get_variable("d{}".format(i),
                                 shape=[N - K, 1],
                                 dtype=dtype,
                                 initializer=tf.random_normal_initializer(),
                                 trainable=False)
        ...
        result = 0
        # Calculate product at each i and sum up
        for i in range(N):
            fi_1 = (tf.linalg.matmul(tf.transpose(p), C[i]) - tf.linalg.matmul(tf.transpose(q), D[i])) * (tf.linalg.matmul(tf.transpose(C[i]), p) - tf.linalg.matmul(tf.transpose(D[i]), q))
